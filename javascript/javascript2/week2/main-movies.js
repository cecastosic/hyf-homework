...
const eighties = movies.reduce(
  (sum, movie) => (movie.year >= 1980 && movie.year <= 1989 ? (sum += 1) : sum),
  0
);
...
const specificTitles = movies.reduce(
  (sum, movie) =>
    movie.title.search(/Surfer/i) >= 0 ||
    movie.title.search(/Alien/i) >= 0 ||
    movie.title.search(/Benjamin/i) >= 0
      ? (sum += 1)
      : sum,
  0
);
...
const goodMovies = movies.reduce(
  (sum, movie) => (movie.tag === "Good" ? (sum += 1) : sum),
  0
);
const averageMovies = movies.reduce(
  (sum, movie) => (movie.tag === "Average" ? (sum += 1) : sum),
  0
);
const badMovies = movies.reduce(
  (sum, movie) => (movie.tag === "Bad" ? (sum += 1) : sum),
  0
);
